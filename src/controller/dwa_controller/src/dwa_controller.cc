...
PLUGINLIB_EXPORT_CLASS(mp::controller::DWAController, nav_core::BaseLocalPlanner)
...
void DWAController::initialize(std::string name, tf2_ros::Buffer* tf, costmap_2d::Costmap2DROS* costmap_ros) {
    if (!isInitialized()) {
        ...
        // 初始化dwa
        dp_ = std::make_shared<DWA>(name, &planner_util_);
...
bool DWAController::setPlan(const std::vector<geometry_msgs::PoseStamped>& orig_global_plan) {
    ...
    //
    latchedStopRotationController_.resetLatching();
    ROS_INFO("Got new plan");
    // 交给dwa局部规划器
    return dp_->setPlan(orig_global_plan);
}
// movebase框架中的execCycle中不断被循环来计算速度
bool DWAController::computeVelocityCommands(geometry_msgs::Twist& cmd_vel) {
    ...
    // 全局规划器的路径变换到局部规划器的路径,并且是否prune_plan
    ...
    if (latchedStopRotationController_.isPositionReached(&planner_util_, current_pose_)) {
        ...
        return latchedStopRotationController_.computeVelocityCommandsStopRotate(
