...
void DWA::reconfigure(dwa_controller::DWAControllerConfig& config) {
    ...
    if (vx_samp <= 0) {
        ROS_WARN(
            "You've specified that you don't want any samples in the x dimension. We'll at least assume that you want "
            "to "
    ...
    if (vy_samp <= 0) {
        ROS_WARN(
            "You've specified that you don't want any samples in the y dimension. We'll at least assume that you want "
            "to "
    ...
    if (vth_samp <= 0) {
        ROS_WARN(
            "You've specified that you don't want any samples in the th dimension. We'll at least assume that you want "
            "to "
...
base_local_planner::Trajectory DWA::findBestPath(const geometry_msgs::PoseStamped& global_pose,
    ...
    if (publish_traj_pc_) {
        ...
        for (std::vector<base_local_planner::Trajectory>::iterator t = all_explored.begin(); t != all_explored.end();
             ++t) {
