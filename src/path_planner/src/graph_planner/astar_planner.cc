...
bool AstarGlobalPlanner::plan(const Point3d& start, const Point3d& goal, Points3d& path, Points3d& expand) {
    ...
    while (!open_list.empty()) {
        ...
        if (closed_list.find(current.id()) != closed_list.end()) continue;
        ...
        for (const auto& motion : motions) {
            ...
            if (closed_list.find(node_new.id()) != closed_list.end()) continue;
            ...
            if (!is_dijkstra_) node_new.set_h(std::hypot(node_new.x() - goal_node.x(), node_new.y() - goal_node.y()));

            // if using GBFS implementation, only consider heuristics cost
            if (is_gbfs_) node_new.set_g(0.0);
